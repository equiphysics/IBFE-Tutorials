...
\begin{itemize}
\item \(\sigmaC\) is often symmetric in standard (non-polar) continua.
\item \(\Pten\) is generally not symmetric even if \(\sigmaC\) is symmetric (because of the \(\Fdef^{-T}\) mapping).
\end{itemize}
...
\medskip
\textbf{Note:} In immersed boundary settings, the surrounding fluid is often incompressible; some formulations rely on that coupling, but solids may still require volumetric control depending on the model and numerics.
...
\begin{columns}[T,onlytextwidth]
  ...
  \medskip
  \textbf{Example (one common choice):}
  \[
  W_{\mathrm{vol}}(J)=\frac{\kappa}{2}(J-1)^2
  \quad (\kappa \text{ large}).
  \]
  \end{column}
